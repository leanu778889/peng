...
$(function(){
	...
	$("#sort").formValidator({onshow:"请输入排行",onfocus:"排行"}).inputValidator({min:0,max:20,onerror:"请填写数字"}).inputValidator({min:1,max:20,onerror:"不能为空或只能是数字"}).regexValidator({regexp:"num1",datatype:"enum",onerror:"只能是数字"});
...
<div class="wrap">
	<div class="nav">
		...
		<form class="J_ajaxForm"   data-role="list" action="admin.php?m=Menu&a=towlevel" method="post" id="myform">
				...
				<tbody class="" id="J_task_main">
					...
					<tr>
						...
						<td>
							...
							<?php if(!empty($menu_p)){ foreach($menu_p as $key =>$value){?>
							    <option <?php if($value['menu_id'] == $pid){ ?> selected <?php }?>value="<?php echo $value['menu_id']?>"><?php echo $value['menu_name']?></option>	
							<?php } }?>	   		
							</select>
						</td>
