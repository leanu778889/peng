...
$(function(){
	...
	$("#J_task_name_input").formValidator({onshow:"请输入菜单名称",onfocus:"菜单"}).inputValidator({min:1,max:20,onerror:"请填写菜单名称"}).ajaxValidator({
	    type : "get",
		url : "",
		data :"m=Menu&a=checkmenu&menuid=<?php echo $menu_one[0]['menu_id']?>",
		datatype : "html",
		async:'false',
	...
	$("#M").formValidator({onshow:"请输入模块",onfocus:"模块（字母或下划线）"}).inputValidator({min:1,max:20,onerror:"请填写模块"}).inputValidator({min:1,max:20,onerror:"不能为空或只能是字母和下划线"}).regexValidator({regexp:"letter_s",datatype:"enum",onerror:"填写不正确"});
	$("#A").formValidator({onshow:"请输入方法",onfocus:"方法（字母或下划线）"}).inputValidator({min:1,max:20,onerror:"请填写方法"}).inputValidator({min:1,max:20,onerror:"不能为空或只能是字母和下划线"}).regexValidator({regexp:"letter_s",datatype:"enum",onerror:"填写不正确"});
    $("#sort").formValidator({onshow:"请输入排行",onfocus:"排行"}).inputValidator({min:0,max:20,onerror:"请填写数字"}).inputValidator({min:1,max:20,onerror:"用户名应该为2-20位之间"}).regexValidator({regexp:"num1",datatype:"enum",onerror:"排行只能是数字"});
...
<div class="wrap">
	<div class="nav">
		...
		<form class="J_ajaxForm" id="myform" data-role="list" action="admin.php?m=Menu&a=editmenu&menuid=<?php echo $menu_one[0]['menu_id']?>" method="post">
				...
				<tbody class="" id="J_task_main">
					<tr>
						...
						<td>
							...
							<input id="J_task_name_input" name="title" class="input length_5 input_hd" value="<?php echo $menu_one[0]['menu_name']?>" type="text">
						</td>
					...
					<tr>
						...
						<td>
						    <select class="select_5" name="p_menu">
							    ...
							    <?php if($menu_one[0]['type']==1){ ?>
							    <option value="0">无法选择菜单</option>	
							    <?php  }elseif($menu_one[0]['type']==2){ ?>
							    <option value="">--请选择菜单--</option>
							            <?php  foreach($menu_p as $key =>$value){ ?>              
							               <option <?php  if($value['menu_id'] == $menu_one[0]['menu_pid'] ){ echo selected;}?> value="<?php echo $value['menu_id']?>"><?php echo $value['menu_name']?></option>	
							                  	<?php  } }elseif($menu_one[0]['type']==3){?>
							            <option value="" selected>--请选择菜单--</option>
							            <?php  foreach($menu_p as $key =>$value){ ?>              
							     
							               <option <?php  if($value['menu_id'] == $menu_one[0]['menu_id'] ){ echo disabled;}?> value="<?php echo $value['menu_id']?>"><?php echo $value['menu_name']?></option>	
					...
					<tr>
						<th>M</th>
						<td>
							<input id="M" name="M" class="input length_5 input_hd" value="<?php echo $menu_one[0]['menu_m']?>" type="text">
						</td>
					...
					<tr>
						<th>A</th>
						<td>
							<input id="A" name="A" class="input length_5 input_hd"  value="<?php echo $menu_one[0]['menu_a']?>" type="text">
						</td>
					...
					<tr>
						...
						<td>
							<ul class="single_list cc">
								<li><label><input name="is_auto" value="1" <?php if($menu_one[0]['menu_state']==1){ echo checked;}?> type="radio">启用</label></li>
								<li><label><input name="is_auto" value="0" <?php if($menu_one[0]['menu_state']==0){ echo checked;}?> type="radio">不启用</label></li>
							</ul>
						</td>
					...
					<tr>
						<th>排序</th>
						<td>
							<input id="sort" name="sort" class="input length_5 input_hd" type="text" value="<?php echo $menu_one[0]['menu_sort']?>">
						</td>
